...
def extract_jobs(job_name: str) -> list[Job]:
    jobs_json: list[dict[str, str]] = get_jobs_list(job_name)

    jobs: dict[int, dict[str, str]] = {}
    for j in jobs_json:
        jobs[int(j['jobid'])] = j

    jobsmedia_json: list[dict[str, str]] = get_jobmedia_list(job_name)
...
def find_chains(jobs: list[Job]) -> list[list[Job]]:
    ...
    for full_idx in full_indexes:
        chains.append(jobs[len(chains):full_idx + 1])
...
def remove_expired_volumes(level: str, storage: str, job: str, dry_run: bool):
    ...
    jobs = extract_jobs(job)
    # for j in jobs:
        # log_info(j)

    chains = find_chains(jobs)
    for idx, jobs_chain in enumerate(chains, start=1):
        log_info(f"Chain №{idx}:")
        for j in jobs_chain:
